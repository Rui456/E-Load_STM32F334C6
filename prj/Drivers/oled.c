...
void OLED_GPIO_Init()
{
    ...
    GPIO_InitStructure.GPIO_Pin = SPIx_MOSI_PIN | SPIx_CLK_PIN;
	...
	GPIO_PinAFConfig(SPIx_GPIO, SPIx_CLK_PINSRC, SPIx_PIN_AF);
	GPIO_PinAFConfig(SPIx_GPIO, SPIx_MOSI_PINSRC, SPIx_PIN_AF);
	...
	SPI_Init(OLED_SPIx, &SPI_InitStructure);  								//根据SPI_InitStruct中指定的参数初始化外设SPIx寄存器
 
	SPI_Cmd(OLED_SPIx, ENABLE); //使能SPI外设
}
...
void OLED_WriteByte(u8 dat)
{
	...
	SPI_SendData8(OLED_SPIx, dat);	//通过外设SPIx发送一个数据
//	SPI_ReceiveData8(OLED_SPIx);
//	OLED_SPIx->DR = dat;			//因为是直接内存赋值，可能会被编译器优化，
